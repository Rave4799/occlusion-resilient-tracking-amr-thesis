...
load_dotenv(dotenv_path="/Users/radmac/occlusion-resilient-tracking-amr-thesis/.env")
...
frames_dir = Path("/Volumes/RAD/pipeline_work/occlusion_scrub/final_occlusion_clip")
frame_files = sorted(frames_dir.glob("*.jpg"))
print(f"Running full chain on {len(frame_files)} frames (genuinely unseen data)...\n")
...
for i, entry in events_fired:
    print(f"  Frame {i}: track {entry['track_id']}, {entry['frames_lost']} frames lost, bin={entry['last_depth_bin']}")

# Also show the depth-bin distribution across all tracks, regardless of firing,
# so we can sanity-check depth is behaving sensibly on this new footage
print(f"\n=== DEPTH BIN DISTRIBUTION (all tracks, all frames) ===")
bin_counts = {"near": 0, "mid": 0, "far": 0, "None": 0}
